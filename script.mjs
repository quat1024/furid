...
const FursonaCheck = {
  init () {
    ...
    function saveState () {
      ...
      if(undoStack.length > 50) { //enlarge the undo stack, why not
        undoStack.shift();
      }
    }
    ...
    $closeDialog.on("click", function () {
      // $container.hide();
      //actually just make it go back to the main screen
      //since there is no actual site underneath
      $drawingArea.show();
      $submissionInfo.show();
      $submitBtn.show();

      $submissionDone.hide();
      $downloadFursona.hide();
      $closeDialog.hide();
